import { Card, CardContent } from '@/components/ui/card'
import { Badge } from '@/components/ui/badge'
import { Button } from '@/components/ui/button'
import { MapPin, Users, Award, Target } from 'lucide-react'
import { useAboutSection } from '@/hooks/useSanity'
import { urlFor } from '@/lib/sanity'
import albertEdenAerial from '@/assets/albert-eden-aerial.jpg'

const AboutSection = () => {
	const { data: aboutData, isLoading } = useAboutSection()

	// Fallback data - this will be used if Sanity isn't connected yet
	const fallbackData = {
		title: 'Experienced Leadership for Albert Eden',
		subtitle: 'About Paul Sun',
		description:
			'With years of experience in business and community service, Paul Sun brings practical solutions and fiscal responsibility to local government.',
		highlights: [
			{
				title: 'Local Resident',
				description:
					"Long-time Albert Eden resident who understands our community's unique needs and challenges.",
				icon: 'MapPin',
			},
			{
				title: 'Community Focused',
				description:
					'Committed to listening to residents and representing their interests at the council table.',
				icon: 'Users',
			},
			{
				title: 'Proven Track Record',
				description:
					'Successful business background with experience in budget management and strategic planning.',
				icon: 'Award',
			},
			{
				title: 'Results Driven',
				description:
					'Focused on delivering measurable outcomes and value for money in all council initiatives.',
				icon: 'Target',
			},
		],
	}

	const content = aboutData || fallbackData
	const communityImage = aboutData?.communityImage
		? urlFor(aboutData.communityImage).width(800).height(500).url()
		: albertEdenAerial

	// Icon mapping for dynamic icons
	const iconMap = {
		MapPin,
		Users,
		Award,
		Target,
	}

	return (
		<section className='py-20 bg-white'>
			<div className='container mx-auto px-6'>
				<div className='grid lg:grid-cols-2 gap-16 items-center max-w-7xl mx-auto'>
					{/* Content */}
					<div>
						<div className='mb-8'>
							<Badge
								variant='secondary'
								className='mb-4 text-campaign-blue font-semibold px-4 py-2'>
								{content.subtitle}
							</Badge>
							<h2 className='text-4xl lg:text-5xl font-bold text-campaign-navy mb-6 leading-tight'>
								{content.title}
							</h2>
							<p className='text-xl text-campaign-gray leading-relaxed mb-6'>
								{content.description}
							</p>
						</div>

						<div className='grid sm:grid-cols-2 gap-6 mb-8'>
							{content.highlights.map((highlight, index) => {
								const IconComponent =
									iconMap[highlight.icon as keyof typeof iconMap] || MapPin
								const borderColors = [
									'border-l-campaign-blue',
									'border-l-campaign-gold',
									'border-l-green-500',
									'border-l-purple-500',
								]
								const iconColors = [
									'text-campaign-blue',
									'text-campaign-gold',
									'text-green-500',
									'text-purple-500',
								]

								return (
									<Card
										key={index}
										className={`border-l-4 ${borderColors[index]} shadow-md`}>
										<CardContent className='p-6'>
											<div className='flex items-center gap-3 mb-3'>
												<IconComponent
													className={`w-6 h-6 ${iconColors[index]}`}
												/>
												<h3 className='font-semibold text-campaign-navy'>
													{highlight.title}
												</h3>
											</div>
											<p className='text-campaign-gray'>
												{highlight.description}
											</p>
										</CardContent>
									</Card>
								)
							})}
						</div>

						<div className='flex flex-col sm:flex-row gap-4'>
							<Button variant='campaign' size='lg' className='px-8'>
								Contact Paul
							</Button>
							<Button variant='campaign-outline' size='lg' className='px-8'>
								Download Policy Document
							</Button>
						</div>
					</div>

					{/* Image */}
					<div className='relative'>
						<img
							src={communityImage}
							alt='Albert Eden local area aerial view'
							className='rounded-2xl shadow-strong w-full h-[500px] object-cover'
						/>
						<div className='absolute inset-0 bg-gradient-to-t from-campaign-navy/60 via-transparent to-transparent rounded-2xl'></div>
						<div className='absolute bottom-6 left-6 right-6'>
							<h3 className='text-white font-bold text-2xl mb-2'>
								Albert Eden Pukutupapa Ward
							</h3>
							<p className='text-blue-100'>
								A vibrant community deserving strong, responsible representation
							</p>
						</div>
					</div>
				</div>
			</div>
		</section>
	)
}

export default AboutSection
